...
const (
	// MNSTestTimeout : run `MNS_TEST_TIMEOUT=true go run max_connection_example.go` to enable timeout test
	// should get lots of `timeout` in log.
	MNSTestTimeout = "MNS_TEST_TIMEOUT"

	// if less than 1024, should get error `no free connections`
	maxConns = 2048
)
...
func main() {
	...
	if isTestTimeout {
		client = ali_mns.NewAliMNSClient(conf.Url,
			...
			ali_mns.Timeout(3),
			ali_mns.MaxConns(maxConns))
	} else {
		client = ali_mns.NewAliMNSClient(conf.Url, conf.AccessKeyId, conf.AccessKeySecret, ali_mns.MaxConns(maxConns))
	}
